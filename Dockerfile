...
ENV HUGO_CHECKSUM c0130f0bf065dbf4930678674d287759b8d7702b2c709c4d15d150513f80199a
...
RUN wget -q https://github.com/gohugoio/hugo/releases/download/v0.36.1/hugo_0.36.1_Linux-64bit.tar.gz && \
    echo "${HUGO_CHECKSUM}  hugo_0.36.1_Linux-64bit.tar.gz" | sha256sum -cs - && \
    tar xfz hugo_0.36.1_Linux-64bit.tar.gz && \
    mv /tmp/hugo /usr/bin && \
    rm -rf /tmp/* && \
    mkdir /site
