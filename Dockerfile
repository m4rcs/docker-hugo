...
ENV HUGO_CHECKSUM c9cf515067f396807161466c9968f10e61f762f49d766215db37b01402ca7ca7
...
RUN wget -q https://github.com/gohugoio/hugo/releases/download/v0.23/hugo_0.23_Linux-64bit.tar.gz && \
    echo "${HUGO_CHECKSUM}  hugo_0.23_Linux-64bit.tar.gz" | sha256sum -cs - && \
    tar xfz hugo_0.23_Linux-64bit.tar.gz && \
    mv /tmp/hugo /usr/bin && \
    rm -rf /tmp/* && \
    mkdir /site
