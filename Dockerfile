...
RUN wget -q https://github.com/gohugoio/hugo/releases/download/v0.37.1/hugo_0.37.1_Linux-64bit.tar.gz && \
    echo "${HUGO_CHECKSUM}  hugo_0.37.1_Linux-64bit.tar.gz" | sha256sum -cs - && \
    tar xfz hugo_0.37.1_Linux-64bit.tar.gz && \
    mv /tmp/hugo /usr/bin && \
    rm -rf /tmp/* && \
    mkdir /site
